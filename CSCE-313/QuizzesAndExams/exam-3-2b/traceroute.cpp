    ...
    for (int ttl = 1; ttl <= MAX_HOPS; ) {
        ...
        while (1) {
            ...
            if (ret == 0) {
                ...
                int bytes = recvfrom(sockfd, recv_buf, sizeof(recv_buf), 0, (sockaddr *)&addr, (socklen_t *)sizeof(addr));
                if (bytes < 0)
                {
                ...
                if (bytes >= 2 * (sizeof(ipheader) + sizeof(icmpheader))){
                    icmpheader * temp = (icmpheader*)recv_buf;
                    ipheader * ip = (ipheader*)recv_buf;

                    if (temp->icmp_type == ICMP_TIME_EXCEEDED && temp->icmp_seq == ttl){ // from pseudo code
                        std::cout << inet_ntoa(ip->iph_sourceip) << std::endl;
                        ttl++;
                        break;
                    } else if(temp->icmp_type == ICMP_ECHO_REPLY){
                        if (temp->icmp_id == getpid()){
                            std::cout << inet_ntoa(ip->iph_sourceip) << std::endl;
                        }
                        exit(-1);
                    } else {
             ...
             */
            if (retry == MAX_RETRY){ // increase ttl break
                ttl++;
                break;
            }
        }
