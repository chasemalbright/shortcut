...
void data_thread_function(BoundedBuffer* bb, string filename, const vector<string>& colnames) {
    ...
    for (int i = 0; i < (int)colnames.size()-1; i++){
        write_to_file(filename,colnames[i] + ", ", i == 0);
    }

    write_to_file(filename, colnames[(int)colnames.size()-1] + "\n");
    ...
    while(1){
        ...
        write_to_file(filename,(col == (int)colnames.size()-1 ? msg + "\n" : msg + ", "));

        col = col + 1;
        col = col % (int)colnames.size();
    }
