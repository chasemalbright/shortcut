...
void worker_thread_function(BoundedBuffer *response_buffer, BoundedBuffer *request_buffer, FIFORequestChannel *channel, int size)
{ // 2
...
int main(int argc, char *argv[])
{
    ...
    {
        char buf1[1024];
        MESSAGE_TYPE msg1 = NEWCHANNEL_MSG;
        chan->cwrite(&msg1, sizeof(MESSAGE_TYPE));
        chan->cread(buf1, 1024);

        FIFORequestChannel *c = new FIFORequestChannel(buf1, FIFORequestChannel::CLIENT_SIDE); // create new channel
